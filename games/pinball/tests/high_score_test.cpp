...
int main(){
    ...
    high_score::show_and_set_high_score_dialog(entry);settle();replace("Riél O'Neil");key(ImGuiKey_Enter);
    ...
    high_score::show_high_score_dialog();settle();replace("Discard this");cancel();
    require(testSettings["0.Name"]=="Updated 9"&&std::string(high_score::highscore_table[0].Name)=="Updated 9","Cancel committed edits");
    ...
    ImGui::DestroyContext();puts("PASS new score names, UTF-8, rename, Backspace, Cancel, bounds and checksum reload");
}
